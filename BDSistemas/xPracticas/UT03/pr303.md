...
while url:
    ...
    if response.status_code == 200:
        ...
        datos = response.json()
        dftemp = pd.json_normalize(datos["results"])
        listadf.append(dftemp)

        url = datos["next"]

dfFinal = pd.concat(listadf, ignore_index=True)
dfFinal
```
...
```python
def info_planetas(url):
    response = requests.get(url)
    datos = response.json()
    return datos.get('name'), datos.get('terrain'), datos.get('population')


df_20 = dfFinal.head(20).copy()

info = []

for url in df_20['homeworld']:
    info.append(info_planetas(url))


df_planetas = pd.DataFrame(info, columns=['planet_name', 'planet_terrain', 'planet_population'])

df_final_20 = pd.concat([df_20, df_planetas], axis=1)
...
df_films = df_final_20.copy()

df_films = df_films.explode('films')

print(f"Filas tras el explode: {len(df_films)}")

# 4. Reemplazamos las URLs por los títulos
# Usamos apply para transformar cada URL en el nombre de la película
print("Traduciendo URLs de películas a títulos...")
df_films['films'] = df_films['films'].apply(obtener_titulo_pelicula)

# 5. Visualizamos el resultado
# Ahora cada fila representa la relación "Personaje - Película"
print("<< OPERACIÓN COMPLETADA >>")
print(df_films[['name', 'films']].head(10))

# Extra: Responder a tu pregunta de ¿cuántos personajes de media hay en cada película?
media_personajes = df_films.groupby('films')['name'].count().mean()
print(f"\nMedia de personajes por película (en este subconjunto): {media_personajes:.2f}")
```

    Filas tras el explode: 68
    Traduciendo URLs de películas a títulos...
    ...
    Media de personajes por película (en este subconjunto): 11.33
